    ...
    uint32_t buf_len, NvmeRequest *req)
{
    ...
    trans_len = MIN(sizeof(*n->elpes) * (NVME_ELPE + 1), buf_len);
    ...
    if (((addr - 0x1000) >> 2) & 1) {
        ...
        if (unlikely(nvme_check_cqid(n, qid))) {
            ...
            nvme_enqueue_event(n, NVME_AER_TYPE_ERROR,
                NVME_AER_INFO_ERR_INVALID_DB_REGISTER, NVME_LOG_ERROR_INFO);
            return;
        }
