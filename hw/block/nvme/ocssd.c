 ...
 *                        Default: 1 (enabled)
 *   sgl_lbal           : If DPTR is an SGL, interpret LBAL as an SGL too.
 *                        Default: 0 (disabled)
 *
 *
 ...
 *     ns=1 group=* punit=* chunk=0 state=OPEN type=RANDOM
 *
 *     # add an open chunk
...
{
    OcssdNamespace *ons = _ons(o, req);
    _dprintf("vector request nlb %d; lbal:\n", req->nlb);
    ...
    NvmeRequest *req, OcssdChunkDescriptor *chk)
{
    ...
    blk_req->blk_offset = ons->chunk_info_offset;
    ...
    if (chk) {
        ...
        qemu_iovec_add(&blk_req->iov, ons->chunk_info, ons->chunk_info_size);
    }
...
static int ocssd_ns_load_chunk_info(OcssdCtrl *o, OcssdNamespace *ons)
{
    ...
    return blk_pread(blk, ons->chunk_info_offset, ons->chunk_info,
        ons->chunk_info_size);
}
...
static void ocssd_reset_cb(void *opaque, int ret)
{
    ...
    if (!ret) {
        ...
        err = ocssd_do_chunk_reset(o, ons, blk_req->slba, mptr, req);
        if (err) {
            req->status = err;
            goto out;
        }
...
static void ocssd_dsm_cb(void *opaque, int ret)
{
    ...
    if (!ret) {
        err = ocssd_do_chunk_reset(o, ons, blk_req->slba, 0x0, req);
        if (err) {
            req->status = err;
            goto out;
        }
...
static uint16_t ocssd_dsm(OcssdCtrl *o, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    if (dsm->attributes & NVME_DSMGMT_AD) {
        ...
        for (int i = 0; i < nr; i++) {
            ...
            OcssdChunkDescriptor *chk = _get_chunk(o, ons, range[i].slba);
            if (!chk) {
                trace_ocssd_err_invalid_chunk(req->cqe.cid,
    ...
    NvmeBlockBackendRequest *blk_req)
{
    ...
    if (!ons->writefail || !ocssd_rw_is_write(req)) {
        return NVME_SUCCESS;
    }
...
static void ocssd_rw_cb(void *opaque, int ret)
{
    ...
    if (!ret) {
        ...
        if (ocssd_rw_is_write(req) && blk_req->blk_offset >= ns->blk_offset &&
...
static uint16_t ocssd_rw(OcssdCtrl *o, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    if (req->nlb > 1) {
        ...
        if (cmd->psdt && params->sgl_lbal) {
            ...
            status = nvme_dma_read_sgl(n, (uint8_t *) req->slba, len, sgl, cmd,
                req);
            if (status) {
                if (nvme_is_error(status, NVME_DATA_SGL_LENGTH_INVALID)) {
                    status = OCSSD_LBAL_SGL_LENGTH_INVALID | NVME_DNR;
                }
    ...
    for (uint32_t i = 0; i < req->nlb; i++) {
        if ((dulbe & (1 << i)) && !ocssd_rw_is_write(req) &&
...
static uint16_t ocssd_io_cmd(NvmeCtrl *n, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    case NVME_CMD_WRITE:
        ...
        int err = nvme_blk_map(n, cmd, req, ocssd_blk_setup_scalar);
        if (err) {
            return err;
        }
...
static uint64_t ocssd_ns_calc_chunk_info_size(OcssdCtrl *o,
    OcssdNamespace *ons)
{
    ...
    Error **errp)
{
    ...
    ons->chunk_info_size = ocssd_ns_calc_chunk_info_size(o, ons);
    ons->chunk_info = g_malloc0(ons->chunk_info_size);

    ons->chunk_info_offset = ns->blk_offset;
    ns->blk_offset += ons->chunk_info_size;

    ns->ns_blks = ocssd_ns_calc_blks(o, ons);
    ns->ns_blks -= (sizeof(OcssdIdentity) + ons->chunk_info_size) /
    ...
    if (params->chunkinfo_fname) {
        ...
        for (int i = 0; i < o->hdr.num_ns; i++) {
            ret = blk_pwrite(o->nvme.conf.blk, ons->chunk_info_offset,
                ons->chunk_info, ons->chunk_info_size, 0);
            if (ret < 0) {
